...
export async function POST(request: NextRequest) {
  try {
    ...
    if (slotId) {
      ...
      // Find client ID first if slotId not provided
    ...
    if (!data || data.length === 0) {
      return NextResponse.json({ message: 'No active booking found to cancel' }, { status: 404 });
    }
    ...
    return NextResponse.json({ success: true, message: 'Call canceled successfully' }, { status: 200 });
